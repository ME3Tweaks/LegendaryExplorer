...
using Gammtek.Conduit.CommandLine.Text;
...
namespace Gammtek.Conduit.CommandLine
{
	...
	{
		private static readonly Lazy<Parser> DefaultInstance = new Lazy<Parser>(() => new Parser(new ParserSettings { HelpWriter = Console.Error }));
		...
		/// <returns>
		///     A <see cref="CommandLine.ParserResult{T}" /> containing an instance of type <typeparamref name="T" /> with parsed values
		///     and a sequence of <see cref="CommandLine.Error" />.
		/// </returns>
		...
		/// <returns>
		///     A <see cref="CommandLine.ParserResult{T}" /> containing the appropriate instance with parsed values as a <see cref="System.Object" />
		///     and a sequence of <see cref="CommandLine.Error" />.
		/// </returns>
		/// <exception cref="System.ArgumentNullException">Thrown if one or more arguments are null.</exception>
