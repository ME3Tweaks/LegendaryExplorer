...
namespace Gammtek.Conduit.Paths
{
   ...
   PathTo   = """ + pathTo + @"""";
			...
			internal static bool TryResolveInnerSpecialDirectory(string normalizedPath, out string resolvedPath, out string failureMessage)
			{
				...
				if (isUNCPath)
				{
					...
					normalizedPath = UNCPathHelper.TranformUNCIntoDriveLetter(normalizedPath, out serverShareStart);
				}

				var pathDirectories = normalizedPath.Split(MiscHelpers.DIR_SEPARATOR_CHAR);
				...
				if (isUNCPath)
				{
					resolvedPath = UNCPathHelper.TranformDriveLetterIntoUNC(resolvedPath, serverShareStart);
				}
			...
			private static bool TryGetAbsolutePath(string pathFrom, string pathTo, out string absolutePath, out string failureMessage)
			{
				...
				var pathFromDirectories = pathFrom.Split(MiscHelpers.DIR_SEPARATOR_CHAR);
				var pathToDirectories = pathTo.Split(MiscHelpers.DIR_SEPARATOR_CHAR);
   ...
   PathTo   = """ + pathTo + @"""";
				...
				absolutePath = partExtractedFromPathFrom + MiscHelpers.DIR_SEPARATOR_STRING + partExtractedFromPathTo;
				failureMessage = null;
