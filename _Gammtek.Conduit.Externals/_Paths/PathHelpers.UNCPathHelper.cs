	...
	partial class PathHelpers
	{
		private static class UNCPathHelper
		{
			...
			internal static bool IsAnAbsoluteUNCPath(string normalizedPath, out int lastIndex)
			{
				...
				// Must begin with "\\"
				if (normalizedPath.IndexOf(MiscHelpers.TWO_DIR_SEPARATOR_STRING, StringComparison.Ordinal) != 0)
				{
					return false;
				}
				...
				var indexFirstSeparator = normalizedPath.IndexOf(MiscHelpers.DIR_SEPARATOR_STRING, twoDirSeparatorLength, StringComparison.Ordinal);
				...
				// Share cannot be "." nor ".."
				var indexSecondSeparator = normalizedPath.IndexOf(MiscHelpers.DIR_SEPARATOR_STRING, indexFirstSeparator + 1, StringComparison.Ordinal);
			...
			internal static bool StartLikeUNCPath(string normalizedPath)
			{
				...
				return normalizedPath.IndexOf(MiscHelpers.TWO_DIR_SEPARATOR_STRING, StringComparison.Ordinal) == 0;
			}

			internal static string TranformDriveLetterIntoUNC(string normalizedPath, string serverShareStart)
			{
				Argument.IsNotNull(nameof(normalizedPath), normalizedPath);
				...
				var uncNormalizedPath = normalizedPath.Remove(0, FakeDriveLetterPrefix.Length).Insert(0, serverShareStart);

				return uncNormalizedPath;
			}

			internal static string TranformUNCIntoDriveLetter(string normalizedPath, out string serverShareStart)
			{
				Argument.IsNotNull(nameof(normalizedPath), normalizedPath);
				...
				var driveLetterNormalizedPath = normalizedPath.Remove(0, lastIndex).Insert(0, FakeDriveLetterPrefix);
