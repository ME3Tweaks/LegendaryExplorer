	...
	public class SFXSaveGameFile : ISerializable, INotifyPropertyChanged
	{
		...
		public ByteOrder ByteOrder
		{
			...
			set
			{
				...
				{
					_byteOrder = value;
					NotifyPropertyChanged("ByteOrder");
				}
			}
		}
		...
		public static void CompareObjects(object obj1, object obj2, string name, Type[] validTypes, HashSet<object> processed, StringBuilder sb)
		{
			...
			{
				sb.AppendLine($"{name}: [NULL] {obj1 ?? "NULL"} != {obj2 ?? "NULL"}");

				return;
			}

			// Ignore Head Morph for time being
			if (name.Contains("MorphHead"))
			{
				return;
			}
			...
			if (type1 != type2)
			{
				sb.AppendLine($"{name}: [TYPE] {type1} != {type2}");
			...
			if (validTypes.Contains(type1))
			{
				...
				sb.AppendLine($"{name}: [VALUE] {obj1} != {obj2}");
			...
			var fields = type1.GetFields(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);
			...
			foreach (var field in fields)
			{
				...
				{
					var collection1 = value1 as ICollection;
					var collection2 = value2 as ICollection;

					if (collection1.Count != collection2.Count)
					{
						sb.AppendLine($"{name}/{field.Name}: [COUNT] {collection1.Count} != {collection2.Count}");
					}

					var count = Math.Max(collection1.Count, collection2.Count);
					var enumerator1 = collection1.GetEnumerator();
					var enumerator2 = collection2.GetEnumerator();

					for (var j = 0; j < count; ++j)
					{
						CompareObjects(enumerator1.MoveNext() ? enumerator1.Current : null,
							enumerator2.MoveNext() ? enumerator2.Current : null,
							$"{name}/{field.Name}/{j}", validTypes, processed, sb);
					}
				}
				else
				{
					...
					{
						sb.AppendLine($"Exceptions: {value1}, {value2}, {name}/{field.Name}");
					}
				}
			}
		}
