...
namespace LegendaryExplorerCore.Gammtek.IO.Converters
{
    /// <summary>
    ...
    public unsafe class EndianConverter
    {
        ...
        /// <summary>
        /// Reverses the data in the specified data array, starting at offset X, for NumItems of ItemSize.
        /// </summary>
        /// <param name="Block"></param>
        /// <param name="offset"></param>
        /// <param name="NumItems"></param>
        /// <param name="ItemSize"></param>
        public static void ReverseBytes(byte[] Block, int offset, int NumItems, int ItemSize)
        {
            int offset1 = offset;
            int offset2 = offset1 + ItemSize - 1;
            for (int i = 0; i < NumItems; i++, offset1 += ItemSize, offset2 += ItemSize) // for every item that needs swapped...
            {
                int offset1a = offset1;
                int offset2a = offset2;
                while (offset1a < offset2a)
                {
                    // Swap
                    byte tmp = Block[offset1a];
                    Block[offset1a] = Block[offset2a];
                    Block[offset2a] = tmp;
                    offset1a++;
                    offset2a--;
                }
