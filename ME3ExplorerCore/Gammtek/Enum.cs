...
namespace ME3ExplorerCore.Gammtek
{
		...
		where TEnum : struct, IComparable, IFormattable
	{
		...
		{
			Argument.IsNotNull(nameof(inputEnumValue), inputEnumValue);
			Argument.IsOfType(nameof(inputEnumValue), inputEnumValue, typeof (Enum));
		...
		public static class DataBinding
		{
			...
			public static IList<IBindableEnum<TEnum>> CreateList(FormatEnumName formatName = null)
			{
				...
				return (from TEnum value in values
					select formatName != null ? new InternalBindableEnum(value, formatName(value)) : new InternalBindableEnum(value)).Cast<IBindableEnum<TEnum>>()
					.ToList();
			}
#endif
			...
			private class InternalBindableEnum : IBindableEnum<TEnum>
			{
				...
				/// <summary>
				///     Gets the name of the internal bindable enum.
				/// </summary>
				public string Name { get; }

				/// <summary>
				///     Gets the value of the internal bindable enum.
				/// </summary>
