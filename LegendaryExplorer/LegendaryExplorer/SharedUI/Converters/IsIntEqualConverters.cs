...
namespace LegendaryExplorer.SharedUI.Converters
{
    ...
    public class IsIntEqualVisibilityConverter : IValueConverter
    {
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            if (value is int i && parameter is string s && int.TryParse(s, out int parsed))
            {
                return i == parsed ? Visibility.Visible : Visibility.Collapsed;
            }
    ...
    public class IsIntNotEqualVisibilityConverter : IValueConverter
    {
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            if (value is int i && parameter is string s && int.TryParse(s, out int parsed))
