...
namespace LegendaryExplorer.Tools.AssetDatabase {
   ...
   public partial class AssetDatabaseWindow : TrackingNotifyPropertyChangedWindowBase {
      ...
      private Tuple<string, string, int, string, bool>
         _currentConvo = new(null, null, -1, null, false); //ConvoName, FileName, export, contentdir, isAmbient
      ...
      public const string CustomListDesc =
         "Custom File Lists allow the database to be filtered so only assets that are in certain files or groups of files are shown. Lists can be saved/reloaded.";
      ...
      private bool IsUsageSelected(object obj) {
         return (lstbx_Usages.SelectedIndex >= 0 && currentView == 1) ||
                (lstbx_MatUsages.SelectedIndex >= 0 && currentView == 2) ||
                (lstbx_AnimUsages.SelectedIndex >= 0 && currentView == 5) ||
                (lstbx_MeshUsages.SelectedIndex >= 0 && currentView == 3) ||
                (lstbx_PSUsages.SelectedIndex >= 0 && currentView == 6) ||
                (lstbx_TextureUsages.SelectedIndex >= 0 && currentView == 4) ||
                (lstbx_GUIUsages.SelectedIndex >= 0 && currentView == 7) ||
      ...
      private bool CanUseAnimViewer(object obj) {
         return currentView == 5 && CurrentGame == MEGame.ME3 && lstbx_Anims.SelectedIndex >= 0 &&
                !((lstbx_Anims.SelectedItem as AnimationRecord)?.IsAmbPerf ?? true);
      }

      private bool IsAnimSequenceSelected() => currentView == 5 && lstbx_Anims.SelectedIndex >= 0 &&
                                               !((lstbx_Anims.SelectedItem as AnimationRecord)?.IsAmbPerf ?? true);
      ...
      private void AssetDB_Loaded(object sender, RoutedEventArgs e) {
         ...
         if (CurrentDBPath != null && CurrentDBPath.EndsWith("zip") && File.Exists(CurrentDBPath) &&
             CurrentGame != MEGame.Unknown && CurrentGame != MEGame.UDK) {
      ...
      /// <returns></returns>
      public static async Task LoadDatabase(string currentDbPath, MEGame game, AssetDB database,
         CancellationToken cancelloadingToken) {
      ...
      private static async Task<AssetDB> ParseDBAsync(MEGame dbgame, string dbpath, string build,
         CancellationToken cancel) {
         try {
            return await Task.Run(() => {
               using ZipArchive archive = new(new FileStream(dbpath, FileMode.Open));
               if (archive.Entries.FirstOrDefault(e => e.Name == $"MasterDB.{dbgame}_{build}.bin") is ZipArchiveEntry
                   entry) {
      ...
      public void SwitchGame(object param) {
         ...
         if (CurrentDBPath != null && File.Exists(CurrentDBPath)) {
            ...
            LoadDatabase(CurrentDBPath, CurrentGame, CurrentDataBase, cancelloading.Token)
               .ContinueWithOnUIThread(prevTask => {
                  if (CurrentDataBase.DatabaseVersion == null || CurrentDataBase.DatabaseVersion != dbCurrentBuild) {
                     var warn = MessageBox.Show(
                        $"This database is out of date (v {CurrentDataBase.DatabaseVersion} versus v {dbCurrentBuild})\nA new version is required. Do you wish to rebuild?",
                     ...
                     IsBusy = false;
                     CurrentOverallOperationText =
                        $"Database generated {CurrentDataBase.GenerationDate} Classes: {CurrentDataBase.ClassRecords.Count} " +
      ...
      private void GoToSuperClass(object obj) {
         ...
         var scidx = CurrentDataBase.ClassRecords.IndexOf(
            CurrentDataBase.ClassRecords.FirstOrDefault(r => r.Class == sClass));
         if (scidx >= 0) {
      ...
      private void OpenPEDefinitionInToolset() {
         ...
         if (record.ElementType is PlotRecordType.Conditional or PlotRecordType.Transition &&
             record.BaseUsage != null) {
      ...
      private void lstbx_Lines_SelectionChanged(object sender, SelectionChangedEventArgs e) {
         ...
         if (currentView == 8 && lstbx_Lines.SelectedIndex >= 0) {
            ...
            if (convo != null) {
               ...
               CurrentConvo = new Tuple<string, string, int, string, bool>(convo.ConvName, fileName,
                  convo.ConvFile.UIndex, CurrentDataBase.ContentDir[directoryKey], convo.IsAmbient);
               ToggleLinePlayback();
               return;
            }
         }
      ...
      private void ToggleRenderMesh() {
         bool showmesh = btn_MeshRenderToggle.IsChecked == true && lstbx_Meshes.SelectedIndex >= 0 &&
                         CurrentDataBase.Meshes[lstbx_Meshes.SelectedIndex].Usages.Count > 0 && currentView == 3;
      ...
      private void ToggleRenderTexture() {
         bool showText = btn_TextRenderToggle.IsChecked == true && lstbx_Textures.SelectedIndex >= 0 &&
                         CurrentDataBase.Textures[lstbx_Textures.SelectedIndex].Usages.Count > 0 && currentView == 4;
         ...
         foreach (var filePath in files) //handle cases of mods/dlc having same file.
         {
            ...
            if (uexpIdx <= textPcc.ExportCount) {
               ...
               if (textExp.Parent != null) {
                  cubemapParent = textExp.Parent.ClassName == "CubeMap"
                     ? selecteditem.TextureName.Substring(textExp.Parent.ObjectName.ToString().Length + 1)
                     : null;
               }
               string indexedName = $"{textExp.ObjectNameString}_{textExp.indexValue - 1}";
               if (textExp.ClassName.StartsWith("Texture") && (textExp.ObjectNameString == selecteditem.TextureName ||
                                                               selecteditem.TextureName == indexedName ||
                                                               (cubemapParent != null && textExp.ObjectNameString ==
                                                                  cubemapParent))) {
      ...
      private void ToggleLinePlayback() {
         bool showAudio = btn_LinePlaybackToggle.IsChecked == true && lstbx_Lines.SelectedIndex >= 0 &&
                          CurrentConvo.Item1 != null && currentView == 8;
         ...
         if (audioPcc != null) {
            if (Path.GetFileNameWithoutExtension(audioPcc.FilePath) ==
                CurrentConvo.Item2) //if switching gender file is already loaded
            {
               var stream = audioPcc.Exports.FirstOrDefault(x =>
                  x.ClassName == "WwiseStream" && x.ObjectNameString.ToLower().Contains(searchWav));
               if (stream != null) {
         ...
         foreach (var filePath in files) //handle cases of mods/dlc having same file.
         {
            ...
            if (currentGame.IsGame1()) {
               var stream = audioPcc.Exports.FirstOrDefault(x =>
                  x.ClassName == "SoundNodeWave" && x.InstancedFullPath.ToLower().EndsWith(searchWav));
               if (stream != null) {
                  SoundpanelWPF_ADB.LoadExport(stream);
                  break;
               }
      ...
      private void SetCRCScan(object obj) {
         if (menu_checkCRC.IsChecked) {
            ...
            var crcdlg =
               MessageBox.Show("Do you want to turn on CRC checking? This will significantly increase scan times.",
                  "Asset Database", MessageBoxButton.YesNo);
      ...
      private void ExportToPSA() {
         if (lstbx_Anims.SelectedItem is AnimationRecord anim && anim.Usages.Any()) {
            ...
            if (pcc.IsUExport(animUIndex) && pcc.GetUExport(animUIndex) is ExportEntry animSeqExp &&
      ...
      private string GetFilePath(int fileListIndex) {
         ...
         return Directory
            .GetFiles(MEDirectories.GetDefaultGamePath(CurrentGame), $"{filename}.*", SearchOption.AllDirectories)
            .FirstOrDefault(f => f.Contains(contentdir));
      }
      ...
      bool LineFilter(object d) {
         if (d is ConvoLine line) {
            ...
            if (cmbbx_filterSpkrs.SelectedIndex >= 0) {
               showthis = string.Equals(line.Speaker, cmbbx_filterSpkrs.SelectedItem.ToString(),
      ...
      private void LoadCustomFileList() {
         ...
         if (d.ShowDialog() == true) {
            ...
            if (!errorlist.IsEmpty()) {
               MessageBox.Show(
                  $"The following files are not in the {CurrentGame} database:\n{string.Join(", ", errorlist)}");
            }
         }
      }
      ...
      public void UpdateSelectedClassUsages() {
         if (ShowAllClassUsages) {
            ...
            SelectedClassUsages = SelectedClass?.Usages.OrderBy(u => u.FileKey)
               .Aggregate(new List<ClassUsage>(), (list, usage) => {
      ...
      private async void ScanGame() {
         ...
         string ShaderCacheName =
            CurrentGame.IsLEGame() ? "RefShaderCache-PC-D3D-SM5.upk" : "RefShaderCache-PC-D3D-SM3.upk";
         List<string> files = Directory.GetFiles(rootPath, "*.*", SearchOption.AllDirectories)
            .Where(s => supportedExtensions.Contains(Path.GetExtension(s.ToLower())) && !s.EndsWith(ShaderCacheName))
            .ToList();
      ...
      private async Task dumpPackages(List<string> files, MEGame game) {
         ...
         ProcessingQueue = new ActionBlock<SingleFileScanner>(x => {
            ...
            Application.Current.Dispatcher.InvokeAsync(() => {
               BusyText =
                  $"Scanned {OverallProgressValue}/{OverallProgressMaximum} files\n\n{GeneratedDB.GetProgressString()}";
