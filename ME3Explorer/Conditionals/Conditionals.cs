...
namespace ME3Explorer
{
    public partial class Conditionals : Form
    {
        ...
        public void LoadFile(string path)
        {
            ...
            {
                temp = Entry[i];
                listBox1.Items.Add(temp.id + " : " + temp.off.ToString("X") + " = " + temp.size.ToString("X"));
            }
            fileStream.Close();
        }
        ...
        {
            return "<rect x=\"" + x + "\" y=\"" + y + "\" width=\"" + w + "\" height=\"" + h + "\" style=\"fill:red;\"/>";
        }
        ...
        {
            return "<line x1=\"" + x1 + "\" y1=\"" + y1 + "\" x2=\"" + x2 + "\" y2=\"" + y2 + "\" style=\"stroke:black; stroke-width:2px;\" />";
        }
        ...
        private void mapToSVGToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (FileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                string Header = "<?xml version=\"1.0\" encoding=\"ISO-8859-1\" standalone=\"no\" ?><!DOCTYPE svg PUBLIC \"-//W3C//DTD SVG 20010904//EN\" \"http://www.w3.org/TR/2001/REC-SVG-20010904/DTD/svg10.dtd\"><svg width=\"" + w + "\" height=\"" + h + "\" xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\">";
                for (int i = 0; i < Header.Length; i++)
                    fileStream.WriteByte((byte)Header[i]);
                for (int i = 0; i < allrefbool.Count; i++)
                {
                    string s = CreateRect(10, i * 40, 100, 30);
                    string s2 = CreateText(15, i * 40 + 20, "Bool #" + allrefbool[i]);
                ...
                for (int i = 0; i < allrefint.Count; i++)
                {
                    ...
                    string s2 = CreateText(99915, i * 40 + 20, "Int #" + allrefint[i]);
                    for (int j = 0; j < s.Length; j++)
                        fileStream.WriteByte((byte)s[j]);
                    for (int j = 0; j < s2.Length; j++)
                        fileStream.WriteByte((byte)s2[j]);
