...
namespace MassEffect2.SaveFormats
{
	...
	public class Player : ISerializable, INotifyPropertyChanged
	{
		...
		public void Serialize(ISerializer stream)
		{
			...
			{
				stream.Serialize(ref _hotKeys);
			}
			
			stream.Serialize(ref _credits);
			stream.Serialize(ref _medigel);
			stream.Serialize(ref _elementZero);
			...
			{
				stream.Serialize(ref _classFriendlyName);
			}
		}
		...
		[LocalizedDisplayName("ElementZero", typeof (Localization.Player))]
		public int ElementZero
		{
			get { return _elementZero; }
			set
			{
				if (value != _elementZero)
				{
					_elementZero = value;
					NotifyPropertyChanged("ElementZero");
				}
			}
		}
